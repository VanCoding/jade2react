...
function Compilation(tree,name){
    this.tree = tree;
    this.name = name;
    this.level = 2;
    this.out = [];
...
Compilation.prototype.compile = function(){
    for(var i = 0; i < this.tree.nodes.length; i++){
        this.renderNode(this.tree.nodes[i]);
    }
    ...
    return [
        "var React = require(\"react\");",
        "var render = require(\"jade2react/lib/render.js\");",
        "var escape = require(\"jade2react/lib/escape.js\");",
    ].concat(this.script,[
        "",
        "",
        "exports.render = function(){",
        "\tvar self = this;",
        "\treturn render(function(__add){",
        this.out.join(""),
        "\t})[0];",
        "}",
        "",
...
Compilation.prototype.renderCode = function(node){
    if(!node.val) return;
    if(node.buffer){
        this.write("__add("+(node.escape?"escape(":"")+node.val+(node.escape?")":"")+");\r\n",0);
    }else{
        this.write(node.val+"\r\n",0);
    }
}
...
Compilation.prototype.renderTag = function(node){
    if(node.name=="main"){
        ...
        if(this.tree.extends){
            this.script.push("module.exports = require(\""+this.tree.extends.replace(/\\/g,"/")+"\").spec;\r\nexports = module.exports;");
        }
        for(var i = 0; i < node.block.nodes.length; i++){
            this.script.push(node.block.nodes[i].val)
        }
    }else{
        this.write("__add("+(React.DOM[node.name]?"React.DOM.":"")+node.name+",",0);
        this.renderAttributes(node);
        if(!node.selfClosing && node.block && node.block.nodes && node.block.nodes.length){
...
Compilation.prototype.renderAttributes = function(node){
    ...
    if(node.attrs){
        ...
        for(var att in attributes){
            this.write((first?"":",")+'"'+att+'":'+attributes[att]);
            first = false;
        }
    }
    this.write("}");
...
Compilation.prototype.renderBlockComment = function(node){
    ...
    this.level++;
    for(var i = 0; i < node.block.nodes.length; i++){
        this.write(node.block.nodes[i].val+"\r\n",0);
    }
    this.write("*/\r\n",-1);
}
